...
namespace IQTools
{
    ...
    public static class ObjectXMLSerializer<T> where T : class // Specify that T must be a class.
    {
        ...
        {
            T serializableObject = LoadFromDocumentFormat(fileStream, new Type[] { typeof(Assets.CheckSums) });
            return serializableObject;
        }
        ...
        private static T LoadFromDocumentFormat(Stream fileStream,System.Type[] extraTypes)
        {
            T serializableObject = null;
           ...
           // {
                ...
                serializableObject = xmlSerializer.Deserialize(fileStream) as T;
           // }
            return serializableObject;
        }
